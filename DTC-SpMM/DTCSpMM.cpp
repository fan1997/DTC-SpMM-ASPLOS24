...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,  int> seg_sort_dequ(int* seg, int*edgeLists, int* nodepointer, int* edgetocol,
                                                                                           int* edgetorow, int* blockPartition, int* blocknum, 
                                                                                           int* row_window_offset,
...
std::vector<torch::Tensor> spmm_forward_improved_ptx_uint8_prefetch_balance_sort_cuda(
	torch::Tensor Rowwindow_offset,
	torch::Tensor TCblocktile_id,
	torch::Tensor TCblock_offset,
	torch::Tensor sparse_AToX_idx,
...
std::vector<torch::Tensor> spmm_forward_ptx_uint8_improved(
  ...
  int embedding_dim = input.size(1);
  return spmm_forward_improved_ptx_uint8_cuda(Rowwindow_offset, 
...
std::vector<torch::Tensor> spmm_forward_origin_clock(
  ...
  CHECK_INPUT(sparse_AToX_idx);
  int embedding_dim = input.size(1);
  return spmm_forward_cuda_origin_clock(Rowwindow_offset, 
                            TCblocktile_id, TCblock_offset, sparse_AToX_idx,
                            num_nodes, num_edges, embedding_dim,
                            input);
}
...
std::vector<torch::Tensor> spmm_balance_forward_ptx_uint8_prefetch(
  ...
  return spmm_balance_forward_cuda_ptx_unit8_prefetch(TCblock_rowid, TCblocktile_id, TCblock_offset, sparse_AToX_idx,
                            tc_count, num_nodes, num_edges, embedding_dim,
                            input, exeplan);
}

std::vector<torch::Tensor> spmm_balance_forward_clock(
    torch::Tensor input,
    torch::Tensor TCblock_rowid,
    torch::Tensor TCblocktile_id,
    torch::Tensor TCblock_offset,
  ...
  int embedding_dim = input.size(1);
  int tc_count = TCblock_rowid.size(0);
  return spmm_balance_clock(TCblock_rowid, TCblocktile_id, TCblock_offset, sparse_AToX_idx,
                            tc_count, num_nodes, num_edges, embedding_dim,
                            input);
}

std::vector<torch::Tensor> spmm_forward_cusparse_impl(
    torch::Tensor input,
  ...
  int embedding_dim = input.size(1);
  return spmm_forward_cusparse(rowoffset, colind, input, num_nodes, num_edges, embedding_dim, alg_id);
}

std::vector<torch::Tensor> spmm_forward_cusparse_blocked_ellpack_impl(
    torch::Tensor input,
	  torch::Tensor ell_colind,
    int num_nodes,
    int block_size,
    int ell_columns
) {
  ...
  int embedding_dim = input.size(1);
  return   spmm_forward_cusparse_blocked_ellpack(ell_colind, input, num_nodes, block_size, ell_columns, embedding_dim);

}

std::vector<torch::Tensor> spmm_forward_sputnik_impl(
    torch::Tensor input,
	  torch::Tensor rowoffset,
	  torch::Tensor colind,
    int num_nodes
  ...
  int num_edges = colind.size(0);
  int embedding_dim = input.size(1);
  return spmm_forward_sputnik(rowoffset, colind, input, num_nodes, num_edges, embedding_dim);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, int> preprocess_gpu(torch::Tensor edgeList_tensor, 
                torch::Tensor nodePointer_tensor, 
                int num_nodes, 
                int blockSize_h,
                int blockSize_w,
    ...
    auto tuple_tensor_blockcnt = seg_sort_dequ(seg_out, edgeList, nodePointer, edgeToColumn, edgeToRow, blockPartition, 
                                               block_num, row_window_offset, blockSize_h, blockSize_w, 
                                               num_nodes, edgeList_tensor.size(0), blockPartition_tensor.size(0));
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed_seconds = end - start;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("run_DTCSpMM", &spmm_forward_ptx_uint8_improved, "DTC-SPMM forward (CUDA)");
  m.def("run_DTCSpMM_balance", &spmm_balance_forward_ptx_uint8_prefetch, "DTC-SpMM balance version forward (CUDA)");
  m.def("forward_origin_clock", &spmm_forward_origin_clock, "DTC-SPMM origin clock forward (CUDA)");
  m.def("DTCSpMM_gcn", &spmm_forward_ptx_uint8_improved_for_gcn, "DTC-SPMM ptx uint8 improved forward gcn (CUDA)");
  m.def("balance_forward_clock", &spmm_balance_forward_clock, "DTC-SpMM balance SPMM ptx uint8 prefetch forward (CUDA)");
  m.def("run_cuSPARSE", &spmm_forward_cusparse_impl, "cusparse SPMM forward (CUDA)");
  m.def("run_Sputnik", &spmm_forward_sputnik_impl, "sputnik SPMM forward (CUDA)");
  m.def("forward_cusparse_bell", &spmm_forward_cusparse_blocked_ellpack_impl, "cusparse SPMM forward with bell(CUDA)");
  
  // backward computation
  m.def("backward", &spmm_forward, "TC-GNN SPMM backward (CUDA)");
  m.def("backward_ptx_uint8_improved", &spmm_forward_ptx_uint8_improved, "DTC-SPMM ptx uint8 improved backward (CUDA)");
  m.def("backward_ptx_uint8_improved_gcn", &spmm_forward_ptx_uint8_improved_for_gcn, "DTC-SPMM ptx uint8 improved backward gcn (CUDA)");
  m.def("balance_backward_ptx_uint8_prefetch", &spmm_balance_forward_ptx_uint8_prefetch, "DTC-SpMM balance SPMM ptx uint8 prefetch backward (CUDA)");
  m.def("backward_cusparse", &spmm_forward_cusparse_impl, "cusparse SPMM backward (CUDA)");
  m.def("backward_sputnik", &spmm_forward_sputnik_impl, "sputnik SPMM backward (CUDA)");
  m.def("backward_cusparse_bell", &spmm_forward_cusparse_blocked_ellpack_impl, "cusparse SPMM back with bell(CUDA)");
}